...
async function SearchBook() {
    ...
    for (var j = 0; j < pathText.length; j++) {
        const web = 'https://hk.finance.yahoo.com/quote/' + pathText[j] + '/';
        ...
        try {
            ...
            const Text = await driver.wait(until.elementsLocated(By.xpath(`//*[@id="quote-header-info"]/div[2]/div[1]/div[1]/h1`)));
            for (const Txt of Text) {
                var Txts = await Txt.getText();
                //console.log(market_priceText);
                Result_Text = Result_Text + Txts + "\r\n";
